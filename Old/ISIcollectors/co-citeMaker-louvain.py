            ...
            if len(pIDs) > 1:
                for i in range(len(pIDs)):
                    ...
                    if grph.has_node(cId1):
                        ...
                        grph.add_node(cId1, val = pDict[cId1], louvain = 0, count = 1)
                    for j in range(i + 1, len(pIDs)):
                        cId2 = pIDs[j]
                        if grph.has_node(cId2):
                            grph.node[cId2]['count'] += 1
                        else:
                            grph.add_node(cId2, val = pDict[cId2], louvain = 0, count = 1)
...
def csvWriteOut(Grph):
    ...
    for n in Grph.nodes():
        attributeCSV.writerow([n, G.node[n]['val'], G.node[n]['count'], G.node[n]['louvain']])
