...
---
<a name="visual"></a>
<a name="visual.blondel"></a>visual.**blondel**(_G, weightParameter=None, communityParameter='community'_):

# Needs to be written

- - -

<a name="visual.btest"></a>visual.**btest**(_quite=False_):

# Needs to be written

- - -

<a name="visual.drop_edges"></a>visual.**drop_edges**(_grph, minWeight=-inf, maxWeight=inf, parameterName='weight', ignoreUnweighted=False, dropSelfLoops=False_):
...
<a name="visual.drop_nodesByCount"></a>visual.**drop_nodesByCount**(_grph, minCount=-inf, maxCount=inf, parameterName='count', ignoreMissing=False_):
...
<a name="visual.drop_nodesByDegree"></a>visual.**drop_nodesByDegree**(_grph, minDegree=-inf, maxDegree=inf, useWeight=True, parameterName='weight', ignoreUnweighted=True_):
...
<a name="visual.filterNonJournals"></a>visual.**filterNonJournals**(_citesLst, invert=False_):

# Needs to be written

- - -

<a name="visual.graphStats"></a>visual.**graphStats**(_G, stats=('nodes', 'edges', 'isolates', 'loops', 'density', 'transitivity'), makeString=True_):

# Needs to be written

- - -

<a name="visual.isiParser"></a>visual.**isiParser**(_isifile_):
...
<a name="visual.modularity"></a>visual.**modularity**(_G, weightParameter=None, communityParameter='community'_):
...
<a name="visual.read_graph"></a>visual.**read_graph**(_edgeList, nodeList=None, directed=False, idKey='ID', eSource='From', eDest='To'_):
...
<a name="visual.recordParser"></a>visual.**recordParser**(_paper_):
...
<a name="visual.write_edgeList"></a>visual.**write_edgeList**(_grph, name, extraInfo=True, progBar=None_):
...
<a name="visual.write_graph"></a>visual.**write_graph**(_grph, name, edgeInfo=True, typing=False, suffix='csv', overwrite=True_):
...
<a name="visual.write_nodeAttributeFile"></a>visual.**write_nodeAttributeFile**(_grph, name, progBar=None_):
