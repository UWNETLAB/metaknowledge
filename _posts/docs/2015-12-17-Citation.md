...
Citation's hashing and equality checking are based on [`getID()`]({{ site.baseurl }}{% post_url /docs/2015-12-17-Citation %}#getID) and use the values of `author`, `year` and `journal`.
...
If the parsing of a citation string fails the attribute `bad` is set to True and the attribute `error` is created to contain the error, which is a [BadCitation]({{ site.baseurl }}{% post_url /docs/2015-12-17-metaknowledge %}#BadCitation) object. If no errors occur `bad` is `False`.
...
Citations can be created by [Records]({{ site.baseurl }}{% post_url /docs/2015-12-17-metaknowledge %}#Record) or by giving the initializer a string containing a WOS style citation.
