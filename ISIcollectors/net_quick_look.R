...
colbar <- rainbow(memb$membership+20)
...
system.time(lay <- layout.fruchterman.reingold(net, niter=1000, coolexp=0.5,
                                               area=vcount(net)^2.3,
                                               repulserad=vcount(net)^2.8))

pdf(file = "net_quickplot.pdf")
set.seed(3952)
par(mar=c(0,0,0,0)+.1) # reduce extra whitespace
plot(net, layout = lay, vertex.size = 3, edge.width = 1, 
